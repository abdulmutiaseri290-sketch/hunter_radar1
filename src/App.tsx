...
// نظام رادار 1 - النسخة الكاملة المستقلة
  ...
  if (!isLoggedIn) {
    return (
      <div style={{ backgroundColor: '#0d0d0d', color: '#FFD700', height: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', fontFamily: 'sans-serif', padding: '20px', textAlign: 'center' }}>
        ...
        <div style={{ backgroundColor: '#1a1a1a', padding: '30px', borderRadius: '25px', border: '1px solid #FFD700', width: '100%', maxWidth: '300px' }}>
          <input 
            type="password" 
            value={pin} 
          ...
          />
          <button onClick={handleLogin} style={{ width: '100%', padding: '15px', backgroundColor: '#FFD700', color: '#000', border: 'none', borderRadius: '12px', fontWeight: 'bold', fontSize: '1.1rem', cursor: 'pointer' }}>دخول للنظام</button>
        </div>
      </div>
    );
  }
        ...
        {activeTab === 'radar' && (
          <div style={{ textAlign: 'center', marginTop: '30px' }}>
            <div style={{ position: 'relative', width: '250px', height: '250px', margin: '0 auto', border: '2px solid #004400', borderRadius: '50%', backgroundColor: '#001a00', overflow: 'hidden' }}>
              ...
              <div style={{ position: 'absolute', top: '40%', left: '50%', width: '10px', height: '10px', backgroundColor: '#FFD700', borderRadius: '50%', boxShadow: '0 0 10px #FFD700' }}></div>
            </div>
