...
// نظام رادار 1 المتكامل - نسخة المدير عبدالمعطي 2026
    ...
    return (
      <div style={{ backgroundColor: '#000', color: '#fff', height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', fontFamily: 'sans-serif', direction: 'rtl' }}>
        <div style={{ textAlign: 'center', border: '1px solid #FFD700', padding: '40px', borderRadius: '30px', background: '#111', width: '300px', boxShadow: '0 0 30px rgba(255, 215, 0, 0.2)' }}>
          ...
          <h1 style={{ color: '#FFD700', margin: '10px 0' }}>رادار 1</h1>
          <input 
            type="password" 
            placeholder="الرمز السري" 
            value={pin}
            onChange={(e) => setPin(e.target.value)}
            style={{ width: '90%', padding: '12px', margin: '20px 0', borderRadius: '10px', border: '2px solid #FFD700', background: '#222', color: '#fff', textAlign: 'center', fontSize: '1.2rem', outline: 'none' }}
          />
          <button onClick={handleLogin} style={{ width: '100%', padding: '12px', backgroundColor: '#FFD700', color: '#000', border: 'none', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer', fontSize: '1.1rem' }}>دخول للنظام</button>
        </div>
      </div>
    );
  }
    ...
    <div style={{ backgroundColor: '#000', color: '#fff', minHeight: '100vh', fontFamily: 'sans-serif', direction: 'rtl', paddingBottom: '80px' }}>
      ...
      {activeTab === 'radar' && (
        <div style={{ textAlign: 'center', padding: '20px' }}>
          ...
          <div style={{ width: '230px', height: '230px', border: '2px solid #004400', borderRadius: '50%', margin: '30px auto', position: 'relative', background: 'radial-gradient(circle, #001a00, #000)', overflow: 'hidden', boxShadow: '0 0 20px #004400' }}>
          ...
          <p style={{ color: '#0f0', fontWeight: 'bold' }}>متصل بالأقمار الصناعية</p>
          <button 
            onClick={() => alert('🚨 تم إرسال نداء الفزعة لجميع الصقارين القريبين!')}
            style={{ background: 'red', color: '#fff', padding: '20px', borderRadius: '15px', border: 'none', width: '90%', fontWeight: 'bold', fontSize: '1.5rem', animation: 'blink 1s infinite', cursor: 'pointer', boxShadow: '0 0 20px rgba(255,0,0,0.5)' }}
          >🚨 فزعة!</button>
        </div>
      )}
        ...
        <div style={{ padding: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <input 
              ...
              style={{ flex: 1, padding: '12px', borderRadius: '10px', border: '1px solid #FFD700', background: '#000', color: '#fff', outline: 'none' }}
            />
            <button onClick={sendMessage} style={{ background: '#FFD700', color: '#000', padding: '0 20px', borderRadius: '10px', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>إرسال</button>
          </div>
        </div>
      )}

      {/* القائمة السفلية (المنيو) */}
      <nav style={{ position: 'fixed', bottom: 0, width: '100%', background: '#111', display: 'flex', justifySelf: 'center', justifyContent: 'space-around', padding: '15px 0', borderTop: '2px solid #FFD700', zIndex: 1000 }}>
